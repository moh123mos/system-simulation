    ...
    <v-container>
      ...
      <v-form @submit.prevent>
        ...
        <div class="text-center">
          <v-btn type="submit" @click="SimulateData" style="text-transform: capitalize"> Simulate</v-btn>
        </div>
      </v-form>
    </v-container>
  </div>
...
serviceTable.forEach((row, index) => {
  ...
  for (let i = 0; i < noCustomers.value; i++) {
    ...
    simulationSheet.getCell(`A${rowIndex}`).value = { formula: `A${rowIndex-1}+1` };
    }
    ...
    if (i === 0) {
      ...
      simulationSheet.getCell(`B${rowIndex}`).value = { formula: `RANDBETWEEN(${minInterarrival.value}, ${maxInterarrival.value})` };
    }
    ...
    if (i === 0) {
      ...
      simulationSheet.getCell(`C${rowIndex}`).value = { formula: `C${rowIndex - 1} + B${rowIndex}` };
    }

    // Random Service Code
    simulationSheet.getCell(`D${rowIndex}`).value = { formula: `RANDBETWEEN(MIN(N6:N${5 + serviceTable.length}), MAX(N6:N${5 + serviceTable.length}))` };

    // Service Title
    simulationSheet.getCell(`E${rowIndex}`).value = { formula: `LOOKUP(D${rowIndex}, $N$6:$N$${5 + serviceTable.length}, $O$6:$O$${5 + serviceTable.length})` };

    // Service Begins
    if (i === 0) {
      simulationSheet.getCell(`F${rowIndex}`).value = 0;
    } else {
      simulationSheet.getCell(`F${rowIndex}`).value = { formula: `MAX(C${rowIndex}, H${rowIndex - 1})` };
    }

    // Service Duration
    simulationSheet.getCell(`G${rowIndex}`).value = { formula: `LOOKUP(D${rowIndex}, $N$6:$N$${5 + serviceTable.length}, $P$6:$P$${5 + serviceTable.length})` };

    // Service End
    simulationSheet.getCell(`H${rowIndex}`).value = { formula: `F${rowIndex} + G${rowIndex}` };

    // System State
    simulationSheet.getCell(`I${rowIndex}`).value = i === 0 ? 'Busy' : { formula: `IF(F${rowIndex} > H${rowIndex - 1}, "Idle", "Busy")` };

    // Customer State
    simulationSheet.getCell(`J${rowIndex}`).value = { formula: `IF(C${rowIndex} < F${rowIndex}, "Waiting", "InService")` };

    // Waiting Time
    simulationSheet.getCell(`K${rowIndex}`).value = { formula: `F${rowIndex} - C${rowIndex}` };
  }
  ...
  simulationSheet.getCell('T5').value = { formula: `COUNT(A6:A${5 + noCustomers.value})` };

  simulationSheet.getCell('S6').value = 'Number of Customers Waiting';
  simulationSheet.getCell('T6').value = { formula: `COUNTIF(J6:J${5 + noCustomers.value}, "Waiting")` };

  simulationSheet.getCell('S7').value = 'Total Waiting Time';
  simulationSheet.getCell('T7').value = { formula: `SUM(K6:K${5 + noCustomers.value})` };
  ...
  simulationSheet.getCell('T10').value = { formula: `COUNTIF(I6:I${5 + noCustomers.value}, "Idle")` };

  simulationSheet.getCell('S11').value = 'Number of Busy States';
  simulationSheet.getCell('T11').value = { formula: `COUNTIF(I6:I${5 + noCustomers.value}, "Busy")` };
...
for (let i = 5; i <= serviceData.length + 3; i++) {
    ...
    const arrivalTimePrevious = (serviceData[i - 5] ? convertToMinutes(serviceData[i - 5].ArrivalTime) : 0);
...
const uniqueInterArrival = [...new Set(interArrivalTimes)].sort((a, b) => a - b);
...
uniqueInterArrival.forEach((value, index) => {
    ...
    simulationSheet.getCell(`N${rowIndex}`).value = { formula: `COUNTIF($AD$4:$AD$${serviceData.length + 3}, M${rowIndex}) / COUNT($AD$4:$AD$${serviceData.length + 3})` }; // Probability
    if(rowIndex!=4)
    simulationSheet.getCell(`O${rowIndex}`).value = { formula: `O${rowIndex-1} + N${rowIndex}` }; // Cumulative Probability
    else
    simulationSheet.getCell(`O${rowIndex}`).value = { formula: `N4` }; // Cumulative Probability
    if(rowIndex!=4)
    simulationSheet.getCell(`P${rowIndex}`).value = { formula: `Q${rowIndex-1}+1` }
    else
    simulationSheet.getCell(`P${rowIndex}`).value = 1
    simulationSheet.getCell(`Q${rowIndex}`).value = { formula: `O${rowIndex}*100` }
});
...
const uniqueDurations = [...new Set(serviceData.map(row => row.Duration))].sort((a, b) => a - b);
uniqueDurations.forEach((value, index) => {
    ...
    simulationSheet.getCell(`T${rowIndex}`).value = { formula: `COUNTIF($AC$4:$AC$${serviceData.length + 3}, S${rowIndex}) / COUNT($AC$4:$AC$${serviceData.length + 3})` };
    if(rowIndex!=4)
    simulationSheet.getCell(`U${rowIndex}`).value = { formula: `U${rowIndex-1} + T${rowIndex}` };
    else
    simulationSheet.getCell(`U${rowIndex}`).value = { formula: `T${rowIndex}` };
    if(rowIndex==4)
    simulationSheet.getCell(`V${rowIndex}`).value = 1; // Start of random digit assignment
    else
    simulationSheet.getCell(`V${rowIndex}`).value =  { formula: `W${rowIndex-1}+1` };
    simulationSheet.getCell(`W${rowIndex}`).value = { formula: `U${rowIndex} * 100` };
});
...
simulationSheet.getCell('G4').value = { formula: `LOOKUP(F4, $V$4:$W${uniqueDurations.length + 3}, $S$4:$S${uniqueDurations.length + 3})` };
...
for (let i = 5; i <= noCustomers.value; i++) {
    ...
    simulationSheet.getCell(`C${i}`).value = { formula: `LOOKUP(B${i},$P$4:$Q$8,$M$4:$M$8)` };
    simulationSheet.getCell(`D${i}`).value = { formula: `D${i-1} + C${i}` };
    simulationSheet.getCell(`E${i}`).value = { formula: `MAX(H${i-1}, D${i})` };
    simulationSheet.getCell(`F${i}`).value = { formula: `RANDBETWEEN(1, 100)` };
    simulationSheet.getCell(`G${i}`).value = { formula: `LOOKUP(F${i}, $V$4:$W${uniqueDurations.length + 3}, $S$4:$S${uniqueDurations.length + 3})` };
...
simulationSheet.getCell('AG4').value = { formula: `COUNT(A4:A${noCustomers.value + 3})` };
simulationSheet.getCell('AF5').value = 'Total Waiting';
simulationSheet.getCell('AG5').value = { formula: `SUM(I4:I${noCustomers.value + 3})` };
...
simulationSheet.getCell('AG7').value = { formula: `COUNTIF(I4:I${noCustomers.value + 3}, ">0") / AG4` };
simulationSheet.getCell('AF8').value = 'Number of Idle';
simulationSheet.getCell('AG8').value = { formula: `COUNTIF(J4:J${noCustomers.value + 3}, ">0")` };
simulationSheet.getCell('AF9').value = 'Number of Busy';
simulationSheet.getCell('AG9').value = { formula: `COUNTIF(J4:J${noCustomers.value + 3}, "0")` };
...
simulationSheet.getCell('AG12').value = { formula: `SUM(G4:G${noCustomers.value + 3})/AG4` };
simulationSheet.getCell('AF13').value = 'Avg Interval';
simulationSheet.getCell('AG13').value = { formula: `SUM(C4:C${noCustomers.value + 3})/(AG4-1)` };
...
uniqueInterArrival.forEach((value, index) => {
    ...
    simulationSheet.getCell(`Q${rowIndex}`).value = { formula: `COUNTIF($AO$4:$AO$${serviceData.length + 3}, P${rowIndex}) / COUNT($AO$4:$AO$${serviceData.length + 3})` }; // Probability
    if(rowIndex!=4)
    simulationSheet.getCell(`R${rowIndex}`).value = { formula: `R${rowIndex-1} + Q${rowIndex}` }; // Cumulative Probability
    else
    simulationSheet.getCell(`R${rowIndex}`).value = { formula: `Q4` }; // Cumulative Probability
    if(rowIndex!=4)
    simulationSheet.getCell(`S${rowIndex}`).value = { formula: `T${rowIndex-1}+1` }
    else
    simulationSheet.getCell(`S${rowIndex}`).value = 1
    simulationSheet.getCell(`T${rowIndex}`).value = { formula: `R${rowIndex}*100` }
});
...
uniqueDurations1.forEach((value, index) => {
  ...
  formula: `COUNTIFS($AM$${4}:$AM$${serviceData.length + 3}, V${rowIndex}, $AN$${4}:$AN$${serviceData.length + 3}, "ser01") / COUNTIF($AN$${4}:$AN$${serviceData.length + 3}, "ser01")`
    ...
    if(rowIndex!=4)
    simulationSheet.getCell(`X${rowIndex}`).value = { formula: `X${rowIndex-1} + W${rowIndex}` };
    else
    simulationSheet.getCell(`X${rowIndex}`).value = { formula: `W${rowIndex}` };
    if(rowIndex==4)
    simulationSheet.getCell(`Y${rowIndex}`).value = 1; // Start of random digit assignment
    else
    simulationSheet.getCell(`Y${rowIndex}`).value =  { formula: `Z${rowIndex-1}+1` };
    simulationSheet.getCell(`Z${rowIndex}`).value = { formula: `X${rowIndex} * 100` };
});
...
uniqueDurations.forEach((value, index) => {
   ...
   formula: `COUNTIFS($AM$${4}:$AM$${serviceData.length + 3}, AB${rowIndex}, $AN$${4}:$AN$${serviceData.length + 3}, "ser02") / COUNTIF($AN$${4}:$AN$${serviceData.length + 3}, "ser02")`
    ...
    if(rowIndex!=4)
    simulationSheet.getCell(`AD${rowIndex}`).value = { formula: `AD${rowIndex-1} + AC${rowIndex}` };
    else
    simulationSheet.getCell(`AD${rowIndex}`).value = { formula: `AC${rowIndex}` };
    if(rowIndex==4)
    simulationSheet.getCell(`AE${rowIndex}`).value = 1; // Start of random digit assignment
    else
    simulationSheet.getCell(`AE${rowIndex}`).value =  { formula: `AF${rowIndex-1}+1` };
    simulationSheet.getCell(`AF${rowIndex}`).value = { formula: `AD${rowIndex} * 100` };
});
...
simulationSheet.getCell(`E${i-1}`).value = { formula: `RANDBETWEEN(1, 100)` };
simulationSheet.getCell(`F${i-1}`).value = 0;
simulationSheet.getCell(`G${i-1}`).value = { formula: `LOOKUP(E4,Y4:Z${uniqueDurations1.length+3},V4:V${uniqueDurations1.length+3})` };
simulationSheet.getCell(`H${i-1}`).value = { formula: `F4+G4` };		
simulationSheet.getCell(`L${i-1}`).value = { formula: `MINUTE(IF(F4<>"",I4-D4,F4-D4))` };	
simulationSheet.getCell(`M${i-1}`).value = { formula: `IF(F4<>"","Server_01","Server_02")` };
  }

simulationSheet.getCell(`A${i}`).value = { formula: `A${i-1}+1` };
simulationSheet.getCell(`B${i}`).value = { formula: `RANDBETWEEN(1, 100)` };
simulationSheet.getCell(`C${i}`).value = { formula: `LOOKUP(B${i},$S$4:$T${uniqueDurations.length + 3},$P$4:$P$${uniqueDurations.length + 3})` };
simulationSheet.getCell(`D${i}`).value = { formula: `D${i-1}+C${i}` };
simulationSheet.getCell(`E${i}`).value = { formula: `RANDBETWEEN(1, 100)` };
simulationSheet.getCell(`F${i}`).value = { formula: `IF(MAX('$H$4':H${i-1})>MAX('$K$4:K${i-1}),"",MAX($H$4:H$4,D${i}))` };
simulationSheet.getCell(`G${i}`).value = { formula: `LOOKUP(E${i},Y4:Z${uniqueDurations.length+3},V4:V${uniqueDurations.length+3})` };
simulationSheet.getCell(`H${i}`).value = { formula: `IF(F${i}<>"",F${i}+G${i},"")` };		
simulationSheet.getCell(`I${i}`).value = { formula: `IF(F${i}<>"","",MAX($K$${i-1}:K${i-1},D${i}))` };
simulationSheet.getCell(`J${i}`).value = { formula: `IF(I${i}<>"",LOOKUP(E${i},$AE$4:$AF$${uniqueDurations.length+3},$AB$4:$AB$${uniqueDurations.length+3}),"")` };

simulationSheet.getCell(`K${i}`).value = { formula: `IF(I${i}<>"",I${i}+J${i},"")` };
simulationSheet.getCell(`L${i}`).value = { formula: `MINUTE(IF(F${i-1}<>"",F${i-1}-D${i-1},I${i-1}-D${i-1}))` };	
simulationSheet.getCell(`M${i}`).value = { formula: `IF(F${i-1}<>"","Server_01","Server_02")` };
}
